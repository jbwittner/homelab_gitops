...
neltharion/               # = hub; in-cluster destination (https://kubernetes.default.svc)
  ...
  apps/
    ...
    forgejo/              # app + Kustomize (manifests bruts : Deployment, CNPG Cluster, PVC 50Gi, Job admin, IngressRoute/cert, IngressRouteTCP SSH) (wave 3) — forge Git + registry packages
...
| 3    | forgejo (forge Git + registry packages — manifests bruts, CNPG Cluster, PVC 50Gi, SSH via Traefik TCP entrypoint 2222) |
| 3    | whoami (test app) |
