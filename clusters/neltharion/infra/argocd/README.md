...
2. **Self-management** : l'`Application` `argocd` pointe sur le **même** dossier (`clusters/neltharion/infra/argocd`) → Argo adopte sa propre config. Ensuite, toute modif passe par Git.

> La convergence est garantie parce que l'apply manuel et l'Application self-managed utilisent **exactement le même dossier** `clusters/neltharion/infra/argocd`.
...
homelab-gitops/
├── bootstrap/
│   └── root-neltharion.yaml             # app-of-apps du cluster (apply manuel UNE fois sur le hub)
├── components/infra/argocd/base/        # bundle Argo PARTAGÉ (agnostique du cluster/hub)
│   ├── kustomization.yaml               # install.yaml pinné (v3.4.3) + patch cmd-params
│   ├── namespace.yaml
│   └── argocd-cmd-params-cm.yaml        # patch ConfigMap argocd-cmd-params-cm
└── clusters/neltharion/infra/argocd/    # dossier auto-contenu (overlay du hub = bootstrap ET self-management)
    ├── argocd.app.yaml                  # Application self-management (path: clusters/neltharion/infra/argocd)
    ├── kustomization.yaml               # base partagée + spécifique-hub ci-dessous (ignore argocd.app.yaml)
...
Argo CD accède au repo via une **deploy key SSH** : lecture seule, scopée à ce repo uniquement, révocable sans toucher au compte GitHub. Le secret est scellé et commité dans `clusters/neltharion/infra/argocd/argocd-repo.sealed-secret.yaml` — il est appliqué en même temps qu'Argo au bootstrap.
...
sed -i '' "s|<COLLER_LA_CLÉ_PRIVÉE_ICI>|$(cat argocd-deploy-key)|" \
  clusters/neltharion/infra/argocd/argocd-repo.secret.yaml
...
kubeseal \
  ...
  --format yaml \
  < clusters/neltharion/infra/argocd/argocd-repo.secret.yaml \
  > clusters/neltharion/infra/argocd/argocd-repo.sealed-secret.yaml

# 5. Committer le sealed (argocd-deploy-key* et *.secret.yaml restent gitignored)
git add clusters/neltharion/infra/argocd/argocd-repo.sealed-secret.yaml
...
> pinnée sur **v3.4.3** dans `components/infra/argocd/base/kustomization.yaml`. Si des pods Argo
...
`root-neltharion` pointe `clusters/neltharion` (recurse + `include: '*.app.yaml'`) → Argo crée toutes les Applications du cluster → déroule les sync-waves. L'Application `argocd` (wave -1) adopte la config déjà déployée à l'étape 1 → passe `Synced` sans rien changer → **self-management acté**.
...
(actifs dans `components/infra/argocd/base/kustomization.yaml`). Reste en roadmap :
...
- Après l'étape 1, tout est déclaratif : le root du cluster (`root-neltharion.yaml`) rejoue toute la stack depuis Git.
...
- [ ] Bumper le tag Argo dans `components/infra/argocd/base/kustomization.yaml` si la compat K8s 1.36 l'exige.