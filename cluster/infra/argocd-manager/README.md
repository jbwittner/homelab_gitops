...
non expirant que porte le Secret de cluster côté hub. Sans lui, aucune Application ciblant ce
cluster distant ne peut être réconciliée.

## Fichiers

- `argocd-manager.appset.yaml` — ApplicationSet, un sous-dossier par cluster **distant**
  (`<cluster>/manifests/`) → une Application `<cluster>-argocd-manager` par spoke, wave `-20`,
  `destination.name: <cluster>`, même `path` que l'apply manuel du bootstrap → adoption immédiate.
...
```bash
kubectl apply -k cluster/infra/argocd-manager/bleu-arcanagos/manifests
...
```yaml
# cluster/infra/argocd/manifests/cluster-bleu-arcanagos.secret.yaml
apiVersion: v1
kind: Secret
metadata:
...
kubeseal --controller-name sealed-secrets --controller-namespace sealed-secrets --format yaml \
  < cluster/infra/argocd/manifests/cluster-bleu-arcanagos.secret.yaml \
  > cluster/infra/argocd/manifests/cluster-bleu-arcanagos.sealed.yaml
rm cluster/infra/argocd/manifests/cluster-bleu-arcanagos.secret.yaml
```

Référencer le `.sealed.yaml` dans `cluster/infra/argocd/manifests/kustomization.yaml`
